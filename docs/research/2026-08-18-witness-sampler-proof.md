...
Branch: `beat-feng-jin`. Companion to the "BREAKTHROUGH CANDIDATE" section of
`2026-08-18-beat-feng-jin-notes.md`. Notation follows Feng-Jin
(arXiv 2410.22267): D is the level base (their eq. (37), D ≥ (2n+1)²/δ),
per-merge arrays A, B are the prefix-maxed level arrays, u, v the residual
weight arrays with entries in {0} ∪ [1, D), C their (max,+)-convolution and
w the weighted witness counts (their Theorem 6.1).
...
3. **Split draw at fine s**: sample the attaining pair (x, y = s−x)
   ...
   c. select rectangle and class ∝ 2^{i+j}·count, select x uniformly in
      ...
      (validated: TV 0.011, mean retries 1.69).
...
project and out of scope for now; the three load-bearing new lemmas are
